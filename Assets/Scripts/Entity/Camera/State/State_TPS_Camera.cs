...
public class State_TPS_Camera : State_Base_Camera {
	//--------------------------------------------------------------------
	// ● メンバ変数
	//--------------------------------------------------------------------
...
/*
		var go = GameObject.Find("Bullet(Clone)");
		if (go != null) {
			ai.transform.position = go.transform.position -
				go.GetComponent<Rigidbody>().velocity.normalized * 0.1f;
...
*/
		...
		if (ai.player.GetComponent<Player>().is_ragdoll)
			offset.y -= 1;
		target_position = ai.player.position +
			ai.player.rotation * offset;
