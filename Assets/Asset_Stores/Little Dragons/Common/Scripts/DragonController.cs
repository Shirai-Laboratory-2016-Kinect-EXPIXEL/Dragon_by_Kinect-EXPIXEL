...
public class DragonController : MonoBehaviour
{
    ...
    void Swimming()
    {
        ...
        //Front RayWater Cast
        if (Physics.Raycast(pivots[2].transform.position, -DragoTransform.up, out WaterHitCenter, dragoHeight * scaleFactor * 2, LayerMask.GetMask("Water")))
        {
        ...
        if (isInWater) //if we hit water
        {
            ...
            if ((Drago_Chest.y < waterlevel && !swim) || (fall && !fly && !isJumping() && Drago_Chest.y - waterlevel<1 ))
            {
                swim = true;
